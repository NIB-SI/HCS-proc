...
umap_no_scaling_dir_rel = config['dim_reduction_visualization']['umap_no_scaling_dir']

# Construct full paths
subsamples_dir = os.path.join(base_path, subsets_filtered_dir_rel)
output_directory = os.path.join(base_path, umap_no_scaling_dir_rel)
...
for subsample_file in subsample_files:
    ...
    umap_embedding = umap_model.fit_transform(features_imputed)
    ...
    embedding_save_path = os.path.join(output_directory, f"{base_file_name}_umap_embedding.npy")
    np.save(embedding_save_path, umap_embedding)
    
    # Construct the output file paths
    png_file_path = os.path.join(output_directory, f"umap_{base_file_name}_concentration.png")
    
    # Save the figures
    plt.savefig(png_file_path, dpi=300)
    plt.close()

print("UMAP plots and embeddings created for all subsamples!")